...
当前可用 `scripts/report-minio-java-parity.py` 生成机器报告。阶段 85 报告显示：对象存储核心 API 59 个中 51 个已经精确同名，剩余 8 个缺失；Admin API 暂无完全缺失项，但还有 8 个别名或部分覆盖项；`*Args` 和 credentials provider 体系仍是后续重点。
