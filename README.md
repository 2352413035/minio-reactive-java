...
- 破坏性 Admin 实验环境：破坏性 Admin 测试只允许在独立可回滚环境中运行，支持独立 lab 配置文件和无凭证准备度审计；默认共享 MinIO 集成测试不会修改危险配置。
