...
| 产品强类型成熟度 | S3 67 / 77、Admin 43 / 128、KMS 7 / 7、STS 4 / 7、Metrics 6 / 6、Health 8 / 8。这个数字表示用户友好 typed 成熟度，不表示路由是否能调用。 |
...
当前已开始补充强业务方法：Health 提供 `isLive()` / `isReady()` 等布尔检查；Metrics/KMS 提供 Prometheus 文本包装和样本解析；STS 提供普通 AssumeRole / WebIdentity / ClientGrants / LDAP 临时凭证解析入口；KMS、Admin IAM、用户组、服务账号、Admin 只读状态摘要以及 S3 版本/分片列表、对象治理、bucket CORS/website/logging/policy status 等子资源提供 typed 模型。其它大量高级管理接口仍保留兼容入口和 raw 兜底，后续按高价值子集逐步增强。
