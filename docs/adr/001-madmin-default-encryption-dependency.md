...
## 阶段 25 再复核（2026-04-24）

本阶段再次选择 **Gate Fail**，但把候选依赖事实更新到当前可查资料：

1. Bouncy Castle 官方文档提供 JDK 1.8+ 与 LTS8 文档入口；Maven Central 显示 `bcprov-jdk18on` 已有 1.84；官方许可证页说明 Bouncy Castle APIs 按 MIT 许可证发布；Javadoc 中可以确认 `Argon2BytesGenerator`，并能找到 `ChaCha20Poly1305` 低层 API。
2. `de.mkammerer:argon2-jvm` 官方仓库说明它通过 JNA 调用 native C 库，许可证为 LGPL v3；Maven Central 显示 2.12 于 2025-03-04 发布。它只解决 Argon2，还需要另选 ChaCha20-Poly1305，因此不适合作为本阶段默认依赖。
3. 自研 Argon2id / ChaCha20-Poly1305 继续被拒绝，因为密码学实现风险、审计成本和长期维护成本都不适合当前 SDK。

因此，本阶段只加固门禁：

- `scripts/madmin-fixtures/check-crypto-gate.sh` 同时检查 `pom.xml` 与源码 import。
- `MadminEncryptionSupportTest` 明确只有 PBKDF2 + AES-GCM 在 Gate Pass 前标记为可解密。
- 默认 madmin-go 加密响应继续通过 `EncryptedAdminResponse` 暴露算法诊断。

### 阶段 25 后续 Gate Pass 条件

未来若要把默认响应解密做成 typed 主路径，必须先补齐：

1. 具体依赖版本和锁定策略。
2. 许可证评审记录。
3. 安全公告和漏洞响应策略。
4. JDK8、JDK17、JDK21、JDK25 测试矩阵。
5. Argon2id + AES-GCM 与 Argon2id + ChaCha20-Poly1305 的 madmin-go fixture 解密测试。
6. FIPS/Provider 是否影响调用方 JVM 全局行为的说明。
7. owner、security reviewer、architect 三方 APPROVE 证据。

任何一项缺失，都继续保持 Gate Fail，不修改 `pom.xml`。

## 阶段 45 Gate Pass 准备但不放行

阶段 45 继续保持 **Gate Fail**。本阶段新增的是放行前清单，而不是依赖实现：

1. 候选依赖必须在实施时重新核对当前版本、许可证、安全公告和传递依赖。
2. 必须记录 owner、security reviewer、architect 三方 APPROVE。
3. 必须证明 JDK8、JDK17、JDK21、JDK25 测试矩阵一致。
4. 必须证明 Argon2id + AES-GCM 与 Argon2id + ChaCha20-Poly1305 默认 fixture 均可解密。
5. 必须保留解密失败时的 `EncryptedAdminResponse` 回退语义。

本阶段不修改 `pom.xml`，不新增 crypto/native/provider 依赖，不更新 `crypto-gate-status.properties` 为 pass。详见 `docs/43-stage45-crypto-gate-pass-prep.md`。
