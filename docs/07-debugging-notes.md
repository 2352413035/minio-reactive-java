...
## 7. 非 S3 协议错误要带 endpoint 上下文

Admin、KMS、STS、Metrics、Health 的错误响应不一定是 S3 XML。当前 SDK 会把这些错误映射到对应的 `ReactiveMinio*Exception`，并保留：
...
这样调用方看到错误时，不需要先去猜“到底调用的是哪个 MinIO 接口”。
