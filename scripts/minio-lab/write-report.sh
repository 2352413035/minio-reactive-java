...
write_minio_lab_report() {
  ...
  local config_enabled quota_enabled remote_enabled tier_enabled batch_enabled
...

  {
    ...
    printf '3. 如果 remote target、tier 或 batch job 探测失败，先查看 MinIO 管理日志，再用独立 lab 的控制台或 `mc admin` 回滚。\n'
    printf '4. 不要把本报告复制到仓库；报告可能包含 lab 端点 和资源名称，但不会包含凭证。\n'
  } > "$report_file"
